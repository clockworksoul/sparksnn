...
// TestFashionMNISTTuned runs the tuned configuration on Fashion-MNIST.
//
// Architecture: 784 → 512 hidden (30% sparse) → 10 output
// Same hyperparameters as the MNIST tuned configuration to demonstrate
// generalization without task-specific tuning.
func TestFashionMNISTTuned(t *testing.T) {
	...
	epochs := 60 // more epochs than MNIST — Fashion-MNIST is harder
	bestAcc := 0.0
	patience := 0
	...
	if bestAcc >= 0.90 {
		t.Logf("🏆 Achieved %.2f%% — exceptional for sparse SNN!", bestAcc*100)
	} else if bestAcc >= 0.87 {
		t.Logf("✓ Achieved %.2f%% — competitive with dense MLP", bestAcc*100)
	} else if bestAcc >= 0.85 {
		t.Logf("✓ Achieved %.2f%% — solid", bestAcc*100)
	} else {
		t.Logf("⚠ Achieved %.2f%% — needs tuning", bestAcc*100)
	}
}
